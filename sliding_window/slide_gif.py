...
def create_sliding_window_gif(image_path, output_path, window_height=20, step_size=10, duration=0.1):
    ...
    for start in range(0, img_height - window_height + 1, step_size):
        ...
        # Combine spectrogram and plot images one above the other with padding
        total_height = frame_img.height + plot_img.height + border_width
        combined_img = Image.new('RGB', (frame_img.width, total_height), (255, 255, 255))
        combined_img.paste(frame_img, (0, 0))
        combined_img.paste(plot_img, (0, frame_img.height + border_width))
    ...
    frames[0].save(output_path, save_all=True, append_images=frames[1:], duration=int(duration * 1000), loop=0)
